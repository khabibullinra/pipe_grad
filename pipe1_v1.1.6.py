...
class grad_Xiao:     
    ...
    def calcAuxiliaryVars(self): #расчет вспомогательных переменных
...
#            def calc_(self,v_s_g, v_s_l, rho_l_PT, mu_l_PT, d_pipe):      #расчет числа Рейнольдса для смеси при пробковом режиме, согласно  Fabre(1994).27961-MS.Advancements in Two-Phase Slug Flow Modeling 
#        v_s = self.v_s_mix
        self.N_Re_mix_slug = self.v_s_mix * self.rho_l_PT * self.d_pipe / self.mu_l_PT
#    def calc_(self,v_s_g, v_s_l):         #Коэффициент, учитывающий насколько максимальная скорость больше средней для фронта Slug, согласно  Fabre(1994).27961-MS.Advancements in Two-Phase Slug Flow Modeling 
...
#    def calc_v_T_B(self):       #Translational Veosity на границе slug региона и региона пуырька Тейлора
...
#        return v_a_g-v_a_l
 ...
 #   def calc_pres_drop_s_l(self):        # Оценка градиента давления для жидкости, для приведенной скорости
      #  f_factor_l=self.f_factor_l
 #       return - self.f_factor_l * self.rho_l_PT * self.v_s_l**2 / (2 * self.d_pipe)
       
 #   def calc_pres_drop_s_g(self):        # Оценка градиента давления для газа, для приведенной скорости
      #  f_factor_g=self.f_factor_g
 #       return -self.f_factor_g * self.rho_g_PT * self.v_s_g**2 / (2 * self.d_pipe)
    ...
    def calc_trans_A(self,dim_h_l):             #Stratified to Nonstratified Transition criterea (если критерий больше либо равен единице, то Nonstratified)
       ...
       # f_large=self.calc_f_large(self.gravity,self.angle,self.d_pipe,self.rho_l_PT,self.rho_g_PT,self.v_s_l,self.v_s_g)
        return self.f_large**2*(1/(1-dim_h_l)**2*(self.dim_v_g**2 * self.dim_S_i / self.dim_A_g))
    ...
    def calc_trans_C(self,dim_h_l):         #Stratified-Smooth to Stratified-Wavy Transition criterea (если больше либо равно нулю, то Stratified-Smooth)
       ...
       # k_large=self.calc_k_large() 
        return self.k_large**2-2 / (self.dim_v_l**0.5 * self.dim_v_g * s**0.5)
    ...
    def calc_trans_D(self,dim_h_l):         #Intermittent to Dispersed-Bubble Transition criterea (если критерий больше либо равен нулю, то Dispersed-Bubble)
        ...
        return self.t_large**2-(8 * self.dim_A_g / (self.dim_S_i * self.dim_v_l**2 * (self.dim_v_l * self.dim_d_l)**(-self.n_power_coeff)))
    ...
    def calc_H_L_gas_core(self):    #Gas Core Liquid Holdup
        f_E = self.calc_f_E_an(self.v_s_g, self.v_s_l)
        return self.v_s_l*f_E/(self.v_s_g+self.v_s_l*f_E)
...
#        return self.v_s_mix
    ...
    def calc_H_L_T_B(self):        #Вычисление холдапа жидкоти в Тейлор бабл регионе из предположения размера пленки, как для стратифайд режима
        return 0.5 + (4/math.pi)*(self.h_f_d - 0.5)*(self.h_f_d-self.h_f_d**2)**0.5 + (1/math.pi)*math.asin(2*self.h_f_d-1)
 
    def calc_v_L_T_B(self):         #Actual film velocity in Taylor Bubble Region
      # H_L_L_S = self.calc_H_L_L_S(v_s_g, v_s_l)
        H_L_T_B = self.calc_H_L_T_B()
       # v_T_B = self.calc_v_T_B(v_s_g, v_s_l)
       # v_L_L_S = self.calc_v_L_L_S(v_s_g, v_s_l)
        self.v_L_T_B = self.v_T_B - (self.v_T_B -self.v_L_L_S)*self.H_L_L_S/ H_L_T_B
    
    def calc_v_g_T_B(self):         #Actual Taylor Bubble velocity in Taylor Bubble Region
        v_s = self.v_s_mix
        H_L_T_B = self.calc_H_L_T_B()
        v_L_T_B = self.calc_v_L_T_B()
        self.v_g_T_B = (v_s - v_L_T_B*H_L_T_B)/(1 - H_L_T_B)
    ...
    def calc_Area_film(self):
        Area_pipe = self.Area_pipe_m
        H_L_T_B = self.calc_H_L_T_B()
        return H_L_T_B*Area_pipe
    
    def calc_Area_gas(self):
        Area_pipe = self.Area_pipe_m
        H_L_T_B = self.calc_H_L_T_B()
        return (1-H_L_T_B)*Area_pipe   
    
    def calc_d_hydr_film(self):         #расчет гидравлического диаметра для пленки
        Area_film = self.calc_Area_film()
        S_f = self.calc_S_f()
        return 4*Area_film/S_f
    
    def calc_d_hydr_gas_bubble(self):         #расчет гидравлического диаметра для пузырька Тейлора
        Area_gas = self.calc_Area_gas()
        S_g = self.calc_S_g()
        S_i = self.calc_S_i()
        return 4*Area_gas/(S_g + S_i) 
    
    def calc_N_Re_film(self):     # Расчет числа Рейнольдса для пленки
        d_hydr_film = self.calc_d_hydr_film()
        v_L_T_B = self.calc_v_L_T_B()
        return self.rho_l_PT*d_hydr_film*math.fabs(v_L_T_B)/self.mu_l_PT
    
    def calc_N_Re_gas_bubble(self):     # Расчет числа Рейнольдса для пузырька Тейлора
        d_hydr_gas_bubble = self.calc_d_hydr_gas_bubble()
       # v_g_T_B = self.calc_v_g_T_B(v_s_g, v_s_l)
        return self.rho_g_PT * d_hydr_gas_bubble * math.fabs(self.v_g_T_B)/self.mu_g_PT
     
    def calc_f_factor_film(self):     # Расчет гидравлического коэффициента потерь на трение для пленки
        if self.calc_N_Re_film() < 2000:
        # режим ламинарный
            return 64*self.calc_N_Re_film( )**(-1)
        else:
            # режим турбулентный
            return 0.184*self.calc_N_Re_film()**(-0.2)
     
    def calc_f_factor_gas_bubble(self):     # Расчет гидравлического коэффициента потерь на трение для пузырька Тейлора
        if self.calc_N_Re_gas_bubble(self)<2000:
        # режим ламинарный
            return 64*self.calc_N_Re_gas_bubble()**(-1)
        else:
            # режим турбулентный
            return 0.184*self.calc_N_Re_gas_bubble( self)**(-0.2)
            
    #Shear Stress
    def calc_tau_w_film(self):      #Liquid Wall Shear Stress for film in Taylor Bubble Region
        f_factor_film = self.calc_f_factor_film() 
        v_L_T_B = self.calc_v_L_T_B()
        return f_factor_film * self.rho_l_PT * self.v_L_T_B * math.fabs(v_L_T_B) / 8
    
    def calc_tau_w_gas_bubble(self):      #Gas Wall Shear Stress for gas bubble in Taylor Bubble Region
        f_factor_gas_bubble = self.calc_f_factor_gas_bubble()
        v_g_T_B = self.calc_v_g_T_B()
    ...
    def combined_momentum_eq_film_reg(self):       #Combined momentum equation in film region
        ...
        while (res>0):
#        h_f_d=h_f_d+0.0001 
            tau_w_film= self.calc_tau_w_film()
            tau_w_gas_bubble = self.calc_tau_w_gas_bubble()
            tau_i = self.calc_tau_i()
            S_f = self.calc_S_f()
            S_g = self.calc_S_g()
            S_i = self.calc_S_i()
            Area_film = self.calc_Area_film()
...
#    def calc_L_S_feet(self):
#        d_pipe_inch = self.d_pipe/0.0254
#        a = math.log(d_pipe_inch) 
#        b = -25.4 + 28.5*a**0.1
#        return math.exp(b)
    
    def calc_L_S_feet_max(self,):
        L_S_feet = self.L_S_feet
        b = math.log(L_S_feet)
        c = 0.5*3.08 + b
        return math.exp(c)
    
#    def calc_L_S(self): #Average Slug Length in meter
#        L_S_feet = self.L_S_feet
